  ...
  try {
    ...
    if (recipient_type === 'all_tenants' || !recipient_type) {
      const tenants = await pool.query(
        "SELECT u.id FROM users u JOIN tenant t ON t.user_id = u.id WHERE t.landlord_id = $1",
        [landlordId]
      );
      ...
      const caretakers = await pool.query(
        "SELECT u.id FROM users u JOIN caretaker c ON c.user_id = u.id WHERE c.landlord_id = $1",
        [landlordId]
      );
