    ...
    try {
      ...
      if (period) {
        ...
        if (!year || !month || month < 1 || month > 12) {
          ...
          return res
            .status(400)
            .json({ error: "Invalid period format. Use YYYY-MM." });
        }
        billingStart = new Date(year, month - 1, 1);
        billingEnd = new Date(year, month, 0);
      } else {
        const now = new Date();
        billingStart = new Date(now.getFullYear(), now.getMonth(), 1);
        billingEnd = new Date(now.getFullYear(), now.getMonth() + 1, 0);
      }
      ...
      for (const lease of leases.rows) {
        const existingInvoice = await client.query(
         ...
         WHERE lease_id = $1 
          ...
          [
            lease.id,
            localDateString(billingStart),
            localDateString(billingEnd),
          ],
        );

        if (existingInvoice.rows.length > 0) {
          const hasPartialPay = await hasPartialPayments(
        ...
        await client.query(
          ...
          [
            ...
            "sent",
            otherCharges > 0
              ? `Includes outstanding balance of R${otherCharges} from previous periods.`
              : null,
          ],
        );
        generated++;
...
router.get("/status", requireAuth, requireLandlord, async (req, res) => {
  try {
    ...
    res.json({
      ...
      current_month: now.toLocaleDateString("en-ZA", {
        month: "long",
        year: "numeric",
      }),
  ...
  async (req, res) => {
    try {
      ...
      const summary = {
        ...
        total_remaining: result.rows.reduce(
          (sum, inv) => sum + Number(inv.remaining_balance),
          0,
        ),
  ...
  async (req, res) => {
    try {
      ...
      if (!check.rows.length) {
        return res
          .status(404)
          .json({ error: "Invoice not found or not owned by you" });
      }
  ...
  async (req, res) => {
    try {
      ...
      res.json({
        ...
        risk_level:
          Number(result.rows[0]?.overdue_balance || 0) > 10000
            ? "high"
            : Number(result.rows[0]?.overdue_balance || 0) > 5000
              ? "medium"
              : "low",
      });
    } catch (err) {
