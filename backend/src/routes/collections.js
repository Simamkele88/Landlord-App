...
router.get("/me", requireAuth, requireTenant, async (req, res) => {
  try {
    ...
    const totalOutstanding = invoices.reduce(
      (sum, inv) => sum + Number(inv.remaining_balance || 0), 0
    );
    ...
    res.json({
      ...
      in_collections: !!collection && ["active", "flagged", "repayment_agreed", "partial_collection"].includes(collection.status),
...
router.get("/", requireAuth, requireLandlord, async (req, res) => {
  try {
    ...
    const summary = {
      ...
      active: accounts.filter(a => a.collections_status === 'active').length,
      partial: accounts.filter(a => a.collections_status === 'partial_collection' || a.partial_invoice_count > 0).length,
      legal: accounts.filter(a => a.collections_status === 'legal').length,
      recovered: accounts.filter(a => a.collections_status === 'recovered').length,
      total_balance: accounts.reduce((sum, a) => sum + Number(a.balance || 0), 0),
      partial_balance: accounts.reduce((sum, a) => sum + Number(a.partial_balance || 0), 0)
    };
...
router.post("/", requireAuth, requireLandlord, async (req, res) => {
  try {
    ...
    const { tenant_id, lease_id, outstanding_balance, notes, invoice_ids } = req.body;
    if (!tenant_id || !lease_id) {
      return res.status(400).json({ error: "tenant_id and lease_id are required" });
    }
    ...
    // Check for active repayment plan
    ...
    if (activePlan.rows.length > 0) {
      return res.status(400).json({
        error: "This tenant has an active repayment plan. Resolve or default the plan before sending to collections.",
    ...
    try {
      ...
      // Check if tenant has partial payments
      ...
      const collResult = await client.query(
        `INSERT INTO collection (
           ...
           $7, $5, NOW(), $8, NOW(), NOW())
         RETURNING *`,
        [tenant_id, lease_id, landlordId, outstanding_balance, req.userId, invoice_ids || [], 
         hasPartial ? 'partial_collection' : 'active', notes || null]
      );
      ...
      await createNotification(
        tenant_id,
        "account_status",
        hasPartial ? "Account in Collections (Partial Payments)" : "Account Sent to Collections",
        notificationMsg,
        collectionId,
        "collection"
      );
 
      await auditLog(req.userId, "CREATE", "collection", collectionId, 
        null, 
      ...
      res.status(201).json({ 
        message: hasPartial ? "Account sent to collections (with partial payments)" : "Account sent to collections", 
  ...
  try {
    ...
    if (String(id).startsWith('inv_')) {
      ...
      const result = await pool.query(
        ...
        RETURNING *`,
        [inv.tenant_id, inv.lease_id, landlordId, inv.remaining_balance, 
         daysOverdue, hasPartial ? 'partial_collection' : 'active', req.userId, note || null]
      );
      ...
      await createNotification(
        inv.tenant_id, "payment_rejected", 
        hasPartial ? "Account in Collections (Partial)" : "Account Sent to Collections",
        notificationMsg,
...
router.put("/:id/status", requireAuth, requireLandlord, async (req, res) => {
  try {
    ...
    const validStatuses = ['active', 'partial_collection', 'collections', 'legal', 'recovered', 'written_off', 'repayment_plan'];
    ...
    await auditLog(req.userId, "UPDATE", "collection", id, 
...
router.get("/partial", requireAuth, requireLandlord, async (req, res) => {
  try {
    ...
    res.json({
      ...
      total_balance: result.rows.reduce((sum, row) => sum + Number(row.outstanding_balance || 0), 0)
    });
  } catch (err) {
