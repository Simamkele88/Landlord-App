...
const INVOICE_TYPE_CONFIG = {
  ...
  fine: { label: "Fine", color: "#9e3a3a", bg: "rgba(158,58,58,0.08)" },
...
function getPaymentPolicy(reliabilityScore, standing, invoice) {
  const remaining = Number(invoice?.remaining_balance || invoice?.amount_due || 0);
  const isHighRisk = reliabilityScore === "high_risk";
  const isBadStanding = ["final_warning", "eviction_notice", "evicted"].includes(standing);
  const isInCollections = invoice?.in_collections === true;
  ...
  if (isHighRisk || isBadStanding) {
    return {
      canPartial: false,
      canPayDirect: true,
      message: `Your current standing does not allow partial payments. Please pay the full outstanding amount of ${fmt(remaining)}.`,
      color: C.red,
    };
  }

  if (reliabilityScore === "moderate_risk") {
    return {
      canPartial: true,
      canPayDirect: true,
      message: "Partial payments are allowed. Please try to keep your account in good standing.",
      color: C.primary,
    };
  }
...
export default function TenantPayments() {
  ...
  function handlePaymentNavigation(route, invoice) {
    const policy = getPaymentPolicy(
      tenant?.reliability_score || "reliable",
      tenant?.standing || "good_standing",
      invoice
    );

    if (!policy.canPayDirect) {
      Alert.alert(
        "Payment Blocked",
        policy.message,
        [{ text: "OK" }]
        ...
        <View style={S.pageHeader}>
          ...
          <View style={[S.scorePill, { borderColor: reliabilityColor(tenantInfo.reliabilityScore) }]}>
            <Text style={[S.scoreText, { color: reliabilityColor(tenantInfo.reliabilityScore) }]}>
              ...
              {tenantInfo.reliabilityScoreValue != null ? ` · ${tenantInfo.reliabilityScoreValue.toFixed(1)}` : ""}
            </Text>
          </View>
        </View>
