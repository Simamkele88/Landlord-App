...
function instalmentStatus(inst) {
  ...
  if (inst.status === "paid")    return { color: C.greenLight, label: "Paid",    icon: "checkmark-circle" };
  if (due < today)               return { color: C.redLight,   label: "Overdue", icon: "alert-circle" };
  return                                { color: C.gold,        label: "Due",     icon: "time" };
}
  ...
  return (
    <View style={[S.instRow, !last && { borderBottomWidth: 1, borderBottomColor: C.border }]}>
      ...
      <View style={{ flex: 1 }}>
        ...
        {isOverdue && (
          <Text style={[S.instMeta, { color: C.redLight }]}>Payment overdue</Text>
        )}
      </View>

      {/* Amount + status */}
      <View style={{ alignItems: "flex-end", gap: 4 }}>
        <Text style={[S.instAmt, isPaid && { color: C.greenLight }]}>{fmt(inst.amount)}</Text>
    ...
    <SafeAreaView style={S.safe}>
      ...
      {!plan ? (
        <View style={{ flex: 1, alignItems: "center", justifyContent: "center", padding: 40 }}>
          <Ionicons name="document-outline" size={40} color="rgba(245,240,232,0.1)" />
          <Text style={{ color: "rgba(245,240,232,0.3)", fontFamily: F.mono, fontSize: 12, marginTop: 12, textAlign: "center" }}>
        ...
        >
          ...
          {isComplete && (
            <View style={[S.pendingBanner, { backgroundColor: "rgba(26,122,74,0.08)", borderColor: "rgba(26,122,74,0.2)" }]}>
              <Ionicons name="checkmark-circle" size={16} color={C.greenLight} />
              <Text style={[S.pendingBannerText, { color: C.greenLight }]}>Plan complete — your account has been cleared.</Text>
            </View>
          )}
          ...
          <View style={S.summaryCard}>
            ...
            <View style={S.ringWrap}>
              ...
              <View style={[S.ringArc, { borderColor: pct >= 100 ? C.greenLight : C.gold }]} />
            </View>
            ...
            <View style={S.summaryRight}>
              ...
              <View style={[S.sumRow, { borderTopWidth: 1, borderTopColor: C.border }]}>
                ...
                <Text style={[S.sumVal, { color: C.greenLight }]}>{fmt(plan.paid_amount)}</Text>
              </View>
              <View style={[S.sumRow, { borderTopWidth: 1, borderTopColor: C.border }]}>
                <Text style={S.sumLabel}>Remaining</Text>
                <Text style={[S.sumVal, { color: C.redLight }]}>{fmt(plan.remaining)}</Text>
              </View>
              <View style={[S.sumRow, { borderTopWidth: 1, borderTopColor: C.border }]}>
          ...
          <View style={S.progressRow}>
            <View style={S.progressTrack}>
              <View style={[S.progressFill, { width: `${pct}%`, backgroundColor: pct >= 100 ? C.greenLight : C.gold }]} />
            </View>
          ...
          {nextDue && !isComplete && (
            <>
              ...
              <View style={[S.nextCard, { borderColor: C.gold + "35" }]}>
                <View style={{ flex: 1 }}>
...
const S = StyleSheet.create({
  ...
  pendingBanner: { flexDirection: "row", alignItems: "center", gap: 10, backgroundColor: "rgba(232,160,18,0.06)", borderRadius: 4, borderWidth: 1, borderColor: "rgba(232,160,18,0.18)", padding: 12, marginBottom: 16 },
  pendingBannerText: { flex: 1, fontSize: 12, color: C.gold, fontFamily: F.dm, lineHeight: 18 },

  summaryCard: { flexDirection: "row", gap: 14, backgroundColor: C.muted2, borderRadius: 6, borderWidth: 1, borderColor: C.border, padding: 16, marginBottom: 10 },
  ringWrap: { width: 80, height: 80, position: "relative", alignItems: "center", justifyContent: "center" },
  ringOuter: { width: 76, height: 76, borderRadius: 38, borderWidth: 5, borderColor: "rgba(245,240,232,0.06)", alignItems: "center", justifyContent: "center" },
  ...
  ringPct: { fontSize: 18, fontFamily: F.bebas, color: C.gold, letterSpacing: 0.5, lineHeight: 22 },
  ringLabel: { fontSize: 8, fontFamily: F.mono, color: "rgba(245,240,232,0.3)", letterSpacing: 1, textTransform: "uppercase" },
  summaryRight: { flex: 1 },
  sumRow: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingVertical: 5 },
  sumLabel: { fontSize: 11, color: "rgba(245,240,232,0.35)", fontFamily: F.mono },
  sumVal: { fontSize: 12, fontWeight: "600", color: C.white, fontFamily: F.dm },

  progressRow: { flexDirection: "row", alignItems: "center", gap: 10, marginBottom: 20 },
  progressTrack: { flex: 1, height: 3, backgroundColor: "rgba(245,240,232,0.06)", borderRadius: 2, overflow: "hidden" },
  progressFill: { height: "100%", borderRadius: 2 },
  progressLabel: { fontSize: 10, fontFamily: F.mono, color: "rgba(245,240,232,0.3)", minWidth: 28 },
  ...
  secSub: { fontSize: 10, color: "rgba(245,240,232,0.25)", fontFamily: F.mono },

  nextCard: { flexDirection: "row", alignItems: "center", backgroundColor: "rgba(232,160,18,0.04)", borderRadius: 4, borderWidth: 1, padding: 14, marginBottom: 20 },
  nextDue: { fontSize: 14, fontWeight: "600", color: C.white, fontFamily: F.dm },
  nextSub: { fontSize: 10, color: "rgba(245,240,232,0.3)", fontFamily: F.mono, marginTop: 2 },
  nextAmt: { fontSize: 22, fontFamily: F.bebas, color: C.gold, letterSpacing: 0.5 },

  card: { backgroundColor: C.muted2, borderRadius: 6, borderWidth: 1, borderColor: C.border, overflow: "hidden", marginBottom: 20 },
  ...
  instDue: { fontSize: 13, fontWeight: "600", color: C.white, fontFamily: F.dm },
  instMeta: { fontSize: 10, color: "rgba(245,240,232,0.3)", fontFamily: F.mono, marginTop: 2 },
  instAmt: { fontSize: 13, fontWeight: "700", color: C.white, fontFamily: F.dm },
  ...
  detailVal: { fontSize: 12, fontWeight: "600", color: C.white, fontFamily: F.dm },

  requestBtn: { marginTop: 20, paddingHorizontal: 20, paddingVertical: 10, borderRadius: 4, borderWidth: 1, borderColor: C.gold },
  requestBtnText: { fontSize: 12, color: C.gold, fontFamily: F.mono, letterSpacing: 0.5 },
});
