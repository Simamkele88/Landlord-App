      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', padding: '2rem', background: C.background }}>
        <div style={{ width: '100%', maxWidth: 420, background: C.card, borderRadius: '8px', border: `1px solid ${C.border}`, padding: '2.5rem 2rem', boxShadow: '0 20px 60px rgba(0,0,0,0.08)' }}>
          <div style={{ textAlign: 'center' }}>
            ...
            <h2 style={{ fontSize: '1.2rem', fontWeight: 700, color: TEXT, fontFamily: F.bebas, letterSpacing: '0.04em', marginBottom: '0.5rem' }}>Check Your Email</h2>
            <p style={{ fontSize: '0.82rem', color: SECONDARY_TEXT, lineHeight: 1.6, marginBottom: '0.3rem' }}>
              If an account exists for <strong style={{ color: TEXT }}>{email}</strong>, a 6-digit reset code has been sent.
            </p>
            <p style={{ fontSize: '0.7rem', color: SECONDARY_TEXT, fontFamily: F.mono, marginBottom: '1.5rem' }}>
              The code expires in 15 minutes. Check your spam folder.
            </p>
            <div style={{ padding: '0.7rem', borderRadius: '3px', background: 'rgba(52,152,219,0.06)', border: '1px solid rgba(52,152,219,0.15)', marginBottom: '1.2rem' }}>
              <p style={{ fontSize: '0.72rem', color: C.blue, lineHeight: 1.4 }}>Enter the code on the reset password page to set a new password.</p>
            </div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', padding: '2rem', background: C.background }}>
      <div style={{ width: '100%', maxWidth: 420 }}>
        ...
        <div style={{ background: C.card, borderRadius: '8px', border: `1px solid ${C.border}`, padding: '2rem', boxShadow: '0 20px 60px rgba(0,0,0,0.08)' }}>
          <h2 style={{ fontSize: '1.1rem', fontWeight: 700, color: TEXT, fontFamily: F.bebas, letterSpacing: '0.04em', marginBottom: '0.3rem' }}>Forgot Password</h2>
          <p style={{ fontSize: '0.78rem', color: SECONDARY_TEXT, marginBottom: '1.5rem' }}>Enter your email and we'll send you a 6-digit reset code.</p>
          ...
          <form onSubmit={handleSubmit}>
            <div style={{ marginBottom: '1rem' }}>
              ...
              <div style={{ position: 'relative' }}>
                <Icon name="mail" size={15} style={{ position: 'absolute', left: '0.8rem', top: '50%', transform: 'translateY(-50%)', color: SECONDARY_TEXT }} />
                <input type="email" value={email} onChange={e => { setEmail(e.target.value); setError(""); }} style={inputStyle} />
              </div>
            </div>

            <button type="submit" disabled={loading} style={{ ...btnStyle, opacity: loading ? 0.6 : 1 }}>
              {loading ? <span style={{ width: 16, height: 16, border: '2px solid rgba(0,0,0,0.2)', borderTopColor: '#fff', borderRadius: '50%', animation: 'spin 0.6s linear infinite' }} /> : "Send Reset Code"}
            </button>
