...
export default function CaretakerComplaints() {
  ...
  const filtered = complaints.filter(c => {
    ...
    const matchSearch = !q || [c.subject, c.filed_by_name, c.against_name, c.category].some(s => (s || "").toLowerCase().includes(q));
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', marginLeft: '0.6rem' }}>
            <button onClick={handleRefresh} disabled={refreshing} style={outlineBtnStyle}>
        ...
        {loading ? (
          ...
          <div style={{ padding: '3rem 1rem', textAlign: 'center' }}>
            ...
            <button onClick={fetchComplaints} style={{ background: 'none', border: 'none', color: '#2471a3', cursor: 'pointer', textDecoration: 'underline', marginTop: '0.5rem' }}>
              Try again
            </button>
          </div>
          ...
          <div style={{ padding: '3rem 1rem', textAlign: 'center', color: '#555' }}>
            ...
            <button onClick={() => { setFilter("All"); setSearch(""); }} style={{ background: 'none', border: 'none', color: '#2471a3', cursor: 'pointer', textDecoration: 'underline', marginTop: '0.5rem' }}>
              Clear filters
            </button>
          </div>
        ) : (
                ...
                {paginatedComplaints.map((c, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Filed By */}
                      <td style={tdStyle}>
                        <div style={{ fontWeight: 500 }}>{c.filed_by_name || "—"}</div>
                      </td>

                      {/* Against */}
                      <td style={tdStyle}>
                        <div style={{ fontWeight: 500, color: c.against_name ? '#151515' : '#555' }}>
                          {c.against_name || "—"}
                        </div>
                      </td>
                      ...
                      {/* Category */}
                      <td style={{ ...tdStyle, textTransform: 'capitalize', fontSize: '11px', color: '#333' }}>
        ...
        {!loading && !error && paginatedComplaints.length > 0 && (
          <div style={{
            ...
            <span>
              Showing {startIndex + 1}-{Math.min(startIndex + pageSize, filtered.length)} of {filtered.length} complaints
            </span>
            {totalPages > 1 && (
