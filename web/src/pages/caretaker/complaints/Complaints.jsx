...
export default function CaretakerComplaints() {
  ...
  const filtered = complaints.filter(c => {
    ...
    if (!q) return true;
    return [c.subject, c.filed_by_name, c.against_name, c.category]
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
              {complaints.length} total · {openCount} active · {escalatedCount} escalated
            </p>
          </div>
                ...
                <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                  ...
                  {filtered.map(c => (
                    <tr key={c.id} className="bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700">
                      ...
                      <td className="px-5 py-4 text-xs text-gray-500 capitalize">{c.category?.replace(/_/g, ' ')}</td>
                      <td className="px-5 py-4"><StatusBadge status={c.status} /></td>
                      <td className="px-5 py-4 text-xs text-gray-400">{timeAgo(c.created_at)}</td>
                      <td className="px-5 py-4">
                        <button onClick={() => navigate(`/caretaker/complaints/${c.id}`)}
