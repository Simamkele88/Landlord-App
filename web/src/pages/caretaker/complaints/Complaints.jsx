...
const STATUS_CONFIG = {
  ...
  "rejected":               { label: "Rejected",           color: 'rgba(245,240,232,0.4)', bg: 'rgba(245,240,232,0.03)', border: '1px solid rgba(245,240,232,0.08)', dot: 'rgba(245,240,232,0.3)' },
};
...
export default function CaretakerComplaints() {
  ...
  function handleRefresh() { setRefreshing(true); fetchComplaints(true); }
  ...
  const S = {
    ...
    toolbar: { display: 'flex', alignItems: 'center', gap: '0.8rem', padding: '0.7rem 1rem', flexWrap: 'wrap', borderBottom: `1px solid ${C.border}` },
    filterBtn: (active) => ({ padding: '0.35rem 0.7rem', borderRadius: '3px', fontSize: '0.68rem', fontWeight: 600, fontFamily: F.mono, letterSpacing: '0.04em', cursor: 'pointer', transition: 'all 0.15s', border: `1px solid ${active ? C.blue : C.border}`, background: active ? 'rgba(58,143,212,0.08)' : 'transparent', color: active ? C.blue : 'rgba(245,240,232,0.4)' }),
    searchWrap: { position: 'relative', marginLeft: 'auto' },
    searchInput: { padding: '0.4rem 0.7rem 0.4rem 2rem', borderRadius: '3px', background: C.black, border: `1px solid ${C.border}`, color: C.white, fontFamily: F.dm, fontSize: '0.74rem', outline: 'none', width: 200 },
  ...
  return (
    <div style={S.container}>
      ...
      <div style={S.headerRow}>
        <div>
          <h1 style={S.title}><Icon name="message-square" size={20} color={C.blue} />Complaints</h1>
          <p style={S.subtitle}>{complaints.length} total · {openCount} active · {escalatedCount} escalated</p>
        </div>
        <button onClick={handleRefresh} disabled={refreshing} style={{
          background: C.blue, color: C.white, border: 'none', padding: '0.45rem 1rem',
          fontSize: '0.68rem', fontWeight: 600, fontFamily: F.dm, letterSpacing: '0.04em',
          borderRadius: '3px', cursor: 'pointer', display: 'inline-flex', alignItems: 'center',
          gap: '0.3rem', opacity: refreshing ? 0.6 : 1,
        }}>
      ...
      {error && (
        <div style={{ padding: '0.7rem 1rem', borderRadius: '3px', marginBottom: '1rem', background: 'rgba(224,90,74,0.06)', border: '1px solid rgba(224,90,74,0.12)', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          ...
          <button onClick={() => fetchComplaints()} style={{ fontSize: '0.68rem', color: C.blue, background: 'none', border: 'none', cursor: 'pointer', fontFamily: F.mono }}>Retry</button>
        </div>
      )}
      ...
      <div style={{ background: C.muted2, border: `1px solid ${C.border}`, borderRadius: '6px', overflow: 'hidden' }}>
              ...
              <tbody>
                ...
                {filtered.map(c => (
                  <tr key={c.id} style={{ transition: 'background 0.15s' }}
                    ...
                    <td style={{ ...S.td, fontSize: '0.68rem', color: 'rgba(245,240,232,0.25)', fontFamily: F.mono }}>{timeAgo(c.created_at)}</td>
                    <td style={S.td}>
                      <button onClick={() => navigate(`/caretaker/complaints/${c.id}`)} style={{ fontSize: '0.66rem', fontWeight: 500, color: C.blue, background: 'none', border: 'none', cursor: 'pointer', fontFamily: F.mono }}
        ...
        <div style={S.footer}><span>Showing <span style={{ color: C.white, fontWeight: 500 }}>{filtered.length}</span> of <span style={{ color: C.white, fontWeight: 500 }}>{complaints.length}</span> complaints</span></div>
      </div>
    </div>
  );
}