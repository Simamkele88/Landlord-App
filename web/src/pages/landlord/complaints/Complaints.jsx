...
import { FiChevronRight, FiSearch, FiRefreshCcw, FiChevronDown } from "react-icons/fi";
      ...
      <div style={{
        ...
        {error && (
          <div style={{
            ...
            margin: '0 1.7rem 1rem', display: 'flex', alignItems: 'center', gap: '0.5rem',
          }}>
            <Icon name="warning" size={16} color={C.redLight} />
            <p style={{ fontSize: '14px', color: C.redLight, flex: 1, margin: 0 }}>{error}</p>
            <button
              onClick={() => fetchComplaints()}
              style={{ fontSize: '13px', color: '#2471a3', background: 'none', border: 'none', cursor: 'pointer' }}
            >
              Retry
            </button>
          </div>
        ...
        <div style={{ border: '1px solid #9a9d9e52', overflow: 'hidden', margin: '0 1.7rem 1.7rem' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={8} style={{ ...tdStyle, textAlign: 'center', padding: '2rem', color: '#555' }}>Loading complaints...</td></tr>
              ) : paginated.length === 0 ? (
                <tr><td colSpan={8} style={{ ...tdStyle, textAlign: 'center', padding: '2rem', color: '#555' }}>No complaints found</td></tr>
              ) : (
                paginated.map((c, index) => {
                  ...
                  return (
                    <tr key={c.id} className="rb-row">
                      ...
                      <td style={tdStyle}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                          <div style={{
                            ...
                            <Icon name="message-square" size={13} color={c.status === 'escalated' ? C.purple : '#f59e0b'} />
                          </div>
                          <div style={{ minWidth: 0 }}>
                            <p style={{
                              fontWeight: 600, color: '#000', fontSize: '13px',
                      ...
                      <td style={tdStyle}>
                        <div style={{ fontWeight: 500, color: '#151515' }}>{c.property_name}</div>
                      ...
                      <td style={tdStyle}>
                        {c.against_name ? (
                          <div style={{ fontWeight: 500, color: '#151515' }}>{c.against_name}</div>
                        ) : (
                          <span style={{ color: '#555', fontStyle: 'italic', fontSize: '11px' }}>—</span>
                        )}
                      </td>
                      {/* Scope */}
                      <td style={tdStyle}>
                        <span style={{ color: '#333', fontSize: '11px', fontFamily: F.mono }}>
          ...
          {!loading && paginated.length > 0 && (
            <div style={{
              ...
              padding: '0.5rem 0.8rem', borderTop: '1px solid #9a9d9e52',
              background: '#e9eced52', fontSize: '13px', color: '#333',
            }}>
              <span>Showing {startIndex + 1}-{Math.min(startIndex + pageSize, filtered.length)} of {filtered.length} complaints</span>
              {filter !== "All" && (
                <button
                  onClick={() => setFilter("All")}
                  style={{ fontSize: '12px', color: '#2471a3', background: 'none', border: 'none', cursor: 'pointer' }}
                >
                  Clear filter
                </button>
              )}
